...
7. **Scaffold (confirm first).** Show the exact `qa/qa.config.json` (from `${CLAUDE_PLUGIN_ROOT}/templates/qa/qa.config.json`, tokens filled, plus interview answers) and `qa/.gitignore` (from `templates/qa/gitignore`). On user confirmation ONLY: write both. Never overwrite an existing file without showing a diff and getting explicit approval.
